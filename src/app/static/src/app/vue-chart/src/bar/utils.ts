...
import {BarchartIndicator, Dict, Filter, FilterOption} from "./types";

export const toFilterLabelLookup = (array: FilterOption[]) =>
...
export const getProcessedOutputData = (data: any[],
                                       ...
                                       xAxisLabels: string[],
                                       xAxisValues: string[]) : BarChartData => {
    ...
    for (const r of data) {
        ...
        if (row[indicator.error_high_column] > maxValuePlusError) {
            maxValuePlusError = row[indicator.error_high_column]
        }
