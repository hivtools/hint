...
describe(`accessibility`, () =>{
    ...
    it(`renders accessibility tags as expected`, () => {
        ...
        const h1 = rendered.findComponent("#accessibility-content")
            .findComponent("h1");

        expectTranslated(h1, "Accessibility on Naomi", "Accessibilité sur Naomi",
            "Acessibilidade do Naomi", store);

        const h2 = rendered.findComponent("#accessibility-content").findAllComponents("h2")

        expectTranslated(h2[0],"How accessible the website is", "Accessibilité du site Web",
            "Acessibilidade do sítio web", store);
        expectTranslated(h2[1], "What we do about known issues", "Traitement des problèmes connus",
            "O que fazemos relativamente aos problemas conhecidos", store);
        expectTranslated(h2[2], "Technical information about this website’s accessibility",
        ...
        expectTranslated(h2[3], "Reporting accessibility issues", "Signalement des problèmes d’accessibilité",
            "Comunicação de problemas de acessibilidade", store);
        expectTranslated(h2[4], "Enforcement procedure", "Procédure d'exécution",
            "Procedimento de aplicação", store);
        expectTranslated(h2[5], "How we test this website", "Comment nous testons ce site Web",
            "De que forma testamos este sítio web", store);
        expectTranslated(h2[6], "Last updated", "Dernière mise à jour",
            "Última atualização", store);

        const paragraphs = rendered.findComponent("#accessibility-content").findAllComponents("p")
        expect(paragraphs.length).toBe(12)
        expectTranslated(paragraphs[11],
    ...
    it(`renders accessibility purpose list tags as expected in French`, () => {
        ...
        const li = rendered.findComponent("#accessibility-content").findComponent("#purpose").findAllComponents("ul li")
    ...
    it(`renders accessibility example list tags as expected in French`, () => {
        ...
        const li = rendered.findComponent("#accessibility-content").findComponent("#example").findAllComponents("ul li")
