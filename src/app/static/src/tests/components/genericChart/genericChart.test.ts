...
describe("GenericChart component", () => {
    ...
    it("sets plotly and table Format based on configured valueFormatColumn selected option when plain number", (done) => {
        const valueFormatMetadata: GenericChartMetadataResponse = {
            "test-chart": {
                datasets: [
                    {id: "dataset1", label: "Dataset 1", url: "/dataset1", table: {}}
        ...
        setTimeout(() => {
            ...
            expect(wrapper.find(GenericChartTable).props("valueFormat")).toBe(",");
            done();
        });
    });
