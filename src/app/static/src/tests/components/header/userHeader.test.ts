...
describe("user header", () => {
    ...
    it("contains Basic steps document links", () => {
        const store = createStore();
        const wrapper = shallowMount(UserHeader, {store, stubs: ["router-link"]});
        expect(wrapper.find("a[href='public/resources/Naomi-Help-Guide.pdf']").text()).toBe("Basic steps");
        ...
        expect(ptWrapper.find("a[href='public/resources/Naomi-Help-Guide.pdf']").text()).toBe("Passos básicos");
    });
