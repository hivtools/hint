...
describe(`privacy`, () =>{
    ...
    it(`renders privacy tags as expected`, () => {
        ...
        const h1 = rendered.findComponent("#privacy-content")
            .findComponent("h1");
        ...
        const h2 = rendered.findComponent("#privacy-content").findAllComponents("h2")

        expectTranslated(h2[0],"What do we collect?", "Ce que nous collectons?",
            "O que é que recolhemos?", store);
        expectTranslated(h2[1], "Information we collect from third parties", "Informations que nous recueillons auprès de tiers",
            "Informação que recolhemos de terceiros", store);
        expectTranslated(h2[2], "How we use your personal data", "Comment nous utilisons vos données personnelles",
            "Como utilizamos os seus dados pessoais", store);
        expectTranslated(h2[3], "Where we store your data", "Où nous stockons vos données",
            "Onde guardamos os seus dados", store);
        expectTranslated(h2[4], "How we keep your data secure", "Comment nous assurons la sécurité de vos données",
            "Como mantemos os seus dados seguros", store);
        expectTranslated(h2[5], "How long will you use my personal data for?", "Combien de temps utiliserez-vous mes données personnelles?",
            "Por quanto tempo irá utilizar os meus dados pessoais durante?", store);
        expectTranslated(h2[6], "Disclosing your information", "Divulgation de vos informations",
            "Divulgação da sua informação", store);
        expectTranslated(h2[7], "Your rights", "Vos droits",
            "Os seus direitos", store);
        expectTranslated(h2[8], "Links to other websites", "Liens vers d'autres sites web",
    ...
    it(`renders privacy preamble list tags as expected`, () => {
        ...
        const li = rendered.findComponent("#privacy-content").findComponent("#preamble").findAllComponents("ul li")
