...
export const readUTCString = (isoUTCString: string) => {
    const [ date, time ] = isoUTCString.split("T");
    const [ year, month, day ] = date.split("-");
    const [ hour, minute, longSecondsWithSuffix ] = time.split(":");
    if (!year || !month || !day || !hour || !minute || !longSecondsWithSuffix) {
        console.error("Invalid UTC string")
    }
    const second = longSecondsWithSuffix.slice(0, 2);
    return { year, month, day, hour, minute, second };
};

export const formatDateTime = (isoUTCString: string) => {
    const { year, month, day, hour, minute, second } = readUTCString(isoUTCString);
...
export const nowTime = () => {
    ...
    const shortMonth = now.getMonth() + 1;
    const month = shortMonth < 10 ? `0${shortMonth}` : `${shortMonth}`;

    const shortDay = now.getDate();
    const day = shortDay < 10 ? `0${shortDay}` : `${shortDay}`;

    const shortHour = now.getHours();
    const hour = shortHour < 10 ? `0${shortHour}` : `${shortHour}`;

    const shortMinute = now.getMinutes();
    const minute = shortMinute < 10 ? `0${shortMinute}` : `${shortMinute}`;

    const shortSecond = now.getSeconds();
    const second = shortSecond < 10 ? `0${shortSecond}` : `${shortSecond}`;

    return { year: now.getFullYear(), month, day, hour, minute, second };
};

export const appendCurrentDateTime = () => {
    const { year, month, day, hour, minute, second } = nowTime();
    return `${year % 10}${month}${day}-${hour}${minute}${second}`;
}
