...
import {actions} from "../../src/store/metadata/actions";
import {Mock} from "vitest";
import * as utils from "../../src/store/plotSelections/utils";
import {FileType} from "../../src/store/surveyAndProgram/surveyAndProgram";
import {MetadataMutations} from "../../src/store/metadata/mutations";
import {PlotStateMutations} from "../../src/store/plotState/mutations";
import {Scale} from "../../src/store/plotState/plotState";
...
describe("Metadata actions", () => {
    ...
    it("commits review input metadata and default selections after successful fetch", async () => {
        ...
        expect(commit.mock.calls[0]).toStrictEqual([
            `reviewInput/${ReviewInputMutation.SetLoading}`, {payload: true}, {root: true}
        ]);
        ...
        expect(commit.mock.calls[4]).toStrictEqual([
            `reviewInput/${ReviewInputMutation.SetLoading}`, {payload: false}, {root: true}
        ]);
