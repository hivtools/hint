...
import FileUpload from "../../../src/components/files/FileUpload.vue";
import ManageFile from "../../../src/components/files/ManageFile.vue";
...
describe("Manage file component", () => {
    ...
    it("renders red text for empty field if required is true", () => {
        ...
        expect(wrapper.find("#required").attributes("class")).toBe("ms-1 text-danger");
    ...
    it("renders text for filled form if required is true", () => {
        ...
        expect(wrapper.find("#required").exists()).toBe(true);
        expect(wrapper.find("#required").text()).toBe("(required)");
        expect(wrapper.find("#required").attributes("class")).toBe("ms-1");
        expect(wrapper.find("#required").attributes("style")).toBe("font-size: small;");
    });
    ...
    it("renders remove link if error is present", async () => {
        ...
        await expectTranslated(wrapper.find("a"), "remove", "supprimer", "remover", (wrapper.vm as any).$store);
    ...
    it("renders slot before filename", () => {
        ...
        expect(slot.element!!.nextElementSibling!!.children[0].classList[0]).toBe("file-name");
    });
