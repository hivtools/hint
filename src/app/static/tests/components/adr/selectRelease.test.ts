...
describe("select release", () => {
    ...
    it("renders select release", async () => {
        ...
        (select.props("options") as typeof expectedOptions).forEach((op, i) => {
            const expectedOp = expectedOptions[i];
            expect(op.id).toBe(expectedOp.id);
            expect(op.label).toBe(expectedOp.label);
            expect(op.customLabel.replace(/ |\n/g, "")).toBe(expectedOp.customLabel.replace(/ |\n/g, ""));
        });
    });
